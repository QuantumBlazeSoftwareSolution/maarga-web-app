...
export default function MapPickerModal({
  ...
  useEffect(() => {
    if (isOpen && mapContainerRef.current && !mapRef.current) {
      ...
      // Initialize map
      mapRef.current = L.map(mapContainerRef.current).setView([startLat, startLng], 18);
  ...
  return (
    <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div 
        ...
        style={{ boxShadow: '10px 10px 20px #bebec0, -10px -10px 20px #ffffff' }}
      >
        {/* Header */}
        <div className="flex items-center justify-between p-6 pb-2">
          <div>
            <h3 className="text-xl font-black text-slate-700">Coordinate Adjustment</h3>
            <p className="text-xs font-medium text-slate-400">Click on the map to re-pin: {stationName}</p>
          </div>
          <button 
            onClick={onClose}
            className="flex h-10 w-10 items-center justify-center rounded-xl text-slate-400 hover:text-rose-500 transition-colors"
            style={{ boxShadow: 'inset 4px 4px 10px #c0c3c8, inset -4px -4px 10px #ffffff' }}
          >
        ...
        <div className="p-6">
          <div 
            ...
            style={{ boxShadow: '6px 6px 12px #c0c3c8, -6px -6px 12px #ffffff' }}
          >
            <div ref={mapContainerRef} style={{ height: '400px', width: '100%', cursor: 'crosshair' }} />
            
            {/* Coordinate Overlay */}
            <div className="absolute bottom-4 left-4 z-1000 rounded-xl bg-white/90 px-4 py-2 shadow-lg backdrop-blur-sm border border-white">
              <div className="flex gap-4">
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Latitude</p>
                  <p className="font-mono text-xs font-bold text-slate-700">{currentCoords.lat}</p>
                </div>
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Longitude</p>
                  <p className="font-mono text-xs font-bold text-slate-700">{currentCoords.lng}</p>
                </div>
              </div>
            </div>
            
            <div className="absolute top-4 right-4 z-1000 rounded-lg bg-emerald-500 px-3 py-1 text-[9px] font-black uppercase tracking-widest text-white shadow-lg animate-pulse">
